...
The Three.js scene must remain self-contained and static-hosting compatible. Do not replace the local imports with a CDN URL. If its module graph or WebGL context fails, `imu-sandbox-bootstrap.js` starts the CSS 3D compatibility renderer so the project remains usable in embedded and restricted browsers. Both renderers import the same engine and axis presentation: Roll rotates in the OLED plane, while Pitch and Yaw provide the 3D tilt. Pointer drag changes Yaw/Pitch, Q/E changes Roll, Arrow keys change Pitch/Yaw, and Space cycles modes. Roll must rotate fixed world-down gravity through OLED-local coordinates; Pitch and Yaw must not alter the sand direction. Keep both display canvases exactly 128x64 with nearest-neighbor rendering.
...
- IMU Sandbox desktop/mobile framing, nonblank WebGL/fallback pixels, Roll gravity at `0deg`, `45deg`, `135deg`, and `+/-180deg`, all three planet magnitudes, 3,000 unique particle cells, and all four modes
