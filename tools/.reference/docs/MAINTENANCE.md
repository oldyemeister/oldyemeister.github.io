...
This document is the internal source of truth for future AI-assisted changes. The `tools/` directory is excluded from the Jekyll website. Do not link this document from public site pages.
...
2. Do not move, rewrite, format, or delete anything under `tools/.reference/demos/` unless the user explicitly requests changes to an original project.
3. `tools/.reference/demos/Laser_Puzzle/` contains the CPUlator C laser puzzle, `tools/.reference/demos/DonkeyKong/` contains the Verilog/Quartus game, and `tools/.reference/demos/imu_sandbox_stm32/` contains the STM32 HAL/C project.
4. Keep `tools/` and `test/` in `_config.yml` exclusions so GitHub Pages does not publish internal sources and maintenance material.
...
- `tools/.reference/documents/`: source reports and presentations retained as non-published project references.
...
The converter reads `tools/.reference/demos/Laser_Puzzle/Laser_Puzzle_CPUlator.c` and writes:
...
The browser version follows `tools/.reference/demos/DonkeyKong/drawmario.v`, not the older copy under `gamelogic1121/`. The active ROM declarations identify these source assets:
...
Jekyll exclusion prevents `tools/` from appearing in the generated Pages site. The maintenance notes and original demo source remain publicly accessible through the GitHub repository even though they are not part of the published website.
